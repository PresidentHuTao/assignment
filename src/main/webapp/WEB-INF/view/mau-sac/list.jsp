...
<div class="container">
    <br>
    <form action="/add-mau-sac" method="post">
        <div class="mb-3">
    ...
    <table class="table table-striped table-dark">
        ...
        <c:forEach items="${list}" var="ms">
            <tr>
                ...
                <td>${ms.tenMau}</td>
                <td>
                    <c:if test="${ms.trangThai == 'on'}">on</c:if>
                    <c:if test="${ms.trangThai == 'off'}">off</c:if>
                </td>
                <td>${ms.ngayTao}</td>
                <td>${ms.ngaySua}</td>
                <td>
                    <a href="/delete-mau-sac/${ms.id}" class="btn btn-danger">Delete</a>
                    <a href="/view-update-mau-sac/${ms.id}" class="btn btn-warning">Update</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
